        ...
        # QUIZ PAGE
            ...
            if not series:
                ...
                for idx, quiz in enumerate(quizzes):
                    ...
                    # Créer les options avec le texte complet
                    options_list = [
                        f"A) {quiz['option_a']}",
                        f"B) {quiz['option_b']}",
                        f"C) {quiz['option_c']}",
                    ...
                    # Convertir les réponses stockées en format complet pour l'affichage
                    default_values = []
                    for ans in st.session_state.quiz_answers[quiz['id']]:
                        if ans == 'A':
                            default_values.append(f"A) {quiz['option_a']}")
                        elif ans == 'B':
                            default_values.append(f"B) {quiz['option_b']}")
                        elif ans == 'C':
                            default_values.append(f"C) {quiz['option_c']}")
                        elif ans == 'D':
                            default_values.append(f"D) {quiz['option_d']}")
                    ...
                    # Extraire juste les lettres (A, B, C, D) pour le stockage
                    selected_letters = [opt[0] for opt in selected]
                    st.session_state.quiz_answers[quiz['id']] = selected_letters
                        ...
                        for idx, (qid, corr) in enumerate(corrections.items()):
                            ...
                            if quiz:
                                with st.expander(f"Question {idx+1}: {corr['question']}"):
                                    # Afficher les réponses de l'utilisateur avec texte complet
                                    user_resp = []
                                    for letter in corr['user_answers']:
                                        if letter == 'A':
                                            user_resp.append(f"A) {quiz['option_a']}")
                                        elif letter == 'B':
                                            user_resp.append(f"B) {quiz['option_b']}")
                                        elif letter == 'C':
                                            user_resp.append(f"C) {quiz['option_c']}")
                                        elif letter == 'D':
                                            user_resp.append(f"D) {quiz['option_d']}")
                                    
                                    # Afficher les bonnes réponses avec texte complet
                                    correct_resp = []
                                    for letter in corr['correct_answers']:
                                        if letter == 'A':
                                            correct_resp.append(f"A) {quiz['option_a']}")
                                        elif letter == 'B':
                                            correct_resp.append(f"B) {quiz['option_b']}")
                                        elif letter == 'C':
                                            correct_resp.append(f"C) {quiz['option_c']}")
                                        elif letter == 'D':
                                            correct_resp.append(f"D) {quiz['option_d']}")
                                    
                                    st.markdown(f"**Votre réponse:** {', '.join(user_resp) if user_resp else 'Non répondue'}")
                                    st.markdown(f"**Réponse correcte:** {', '.join(correct_resp)}")
