...
def add_quiz_pending(question, opt_a, opt_b, opt_c, opt_d, reponses, explication, categorie):
    """Ajouter un quiz en attente d'approbation"""
    db.query(
        'INSERT INTO quiz_pending (question, option_a, option_b, option_c, option_d, reponses_correctes, explication, categorie) VALUES (?, ?, ?, ?, ?, ?, ?, ?)',
        (sanitize_input(question), sanitize_input(opt_a), sanitize_input(opt_b), 
         sanitize_input(opt_c), sanitize_input(opt_d), reponses, sanitize_input(explication), categorie)
    )

def get_pending_quizzes():
    """Récupérer les quizzes en attente"""
        ...
        with tab2:
            ...
            if st.button("S'Inscrire", use_container_width=True, type="primary"):
                if not all([nom, prenom, email, username, password]):
                    ...
                    st.info("Utilisez vos identifiants pour vous connecter.")
                else:
                    st.error("❌ Erreur")
...
# ========== APPLICATION PRINCIPALE ==========
    ...
    with st.sidebar:
        ...
        if is_admin:
            ...
            if st.button("🔧 Accès Admin"):
                # Vérifier si l'utilisateur est dans les admins secrets
                try:
                    admins = st.secrets.get("admins", {})
                    if user['email'].lower() in admins:
                        ...
                        st.error("❌ Accès refusé - non administrateur")
                except:
    ...
    # ========== MODE ADMIN ==========
        ...
        # SÉRIES
            ...
            st.markdown("### ➕ Ajouter Quiz Manuellement")
        ...
        # IMPORTER
            ...
            if uploaded:
                try:
                    ...
                    if series_list:
                        selected = st.selectbox("Ajouter à quelle série?", [(s['id'], s['nom']) for s in series_list], format_func=lambda x: x[1])
                        
                        if st.button("Ajouter à la série", use_container_width=True, type="primary"):
                            count = 0
                            for _, row in df.iterrows():
                                add_quiz_pending(row['question'], row['a'], row['b'], 
                                               row['c'], row['d'], row['reponses_correctes'], 
                                               row['explication'], selected[1])
                                count += 1
            ...
            st.divider()
            st.markdown("### ✅ Quizzes en Attente d'Approbation")
            
            pending = get_pending_quizzes()
            
            if pending:
                st.write(f"**{len(pending)} quizzes à examiner**")
                
                for idx, pend in enumerate(pending):
                    st.markdown(f"**Q{idx+1}: {pend['question']}**")
                    st.markdown(f"Catégorie: {pend['categorie']}")
                    st.markdown(f"Options: A) {pend['option_a']} | B) {pend['option_b']} | C) {pend['option_c']} | D) {pend['option_d']}")
                    st.markdown(f"Réponses correctes: {pend['reponses_correctes']}")
                    st.markdown(f"Explication: {pend['explication']}")
                    
                    col1, col2, col3 = st.columns(3)
                    with col1:
                        if st.button("✅ Approuver", key=f"app_{pend['id']}_{idx}", use_container_width=True):
                            # Créer une série si elle n'existe pas
                            series = db.fetch_one('SELECT id FROM series WHERE nom = ?', (pend['categorie'],))
                            if not series:
                                add_series(pend['categorie'], f"Série {pend['categorie']}", 50)
                                series = db.fetch_one('SELECT id FROM series WHERE nom = ?', (pend['categorie'],))
                            
                            approve_quiz(pend['id'], series['id'])
                            st.success("✅ Quiz approuvé et ajouté à la série!")
                            st.rerun()
