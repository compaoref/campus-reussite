    ...
    if not st.session_state.show_results:
        ...
        with col2:
            if st.button("✅ Valider", use_container_width=True, type="primary"):
                ...
                if selected_option == row['reponse']:
                    st.success(f"🎯 Bravo {st.session_state.user_name}! C'est correct !")
                    st.balloons()
                else:
        ...
        for i, row in df_quiz.iterrows():
            ...
            details_data.append({
                ...
                "Explication": row['explication'][:60] + "..."
            })
        ...
        with col2:
            if st.button("📥 Télécharger mon résultat", use_container_width=True):
                csv = details_df.to_csv(index=False)
                st.download_button(
                    label="💾 Télécharger CSV",
                    data=csv,
                    file_name=f"resultats_quiz_{st.session_state.user_name}.csv",
                    mime="text/csv"
                )
...
<div style="text-align: center; color: #6b7280; font-size: 0.85rem; margin-top: 3rem; padding: 2rem; border-top: 1px solid #e5e7eb;">
    <p>© 2024 Campus Réussite — Plateforme d'apprentissage | Version 2.0</p>
</div>
""", unsafe_allow_html=True)
