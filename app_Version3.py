"""
🎓 CAMPUS RÉUSSITE v8.2 FINAL (sécurisé)
Modifs de sécurité:
- PBKDF2-HMAC-SHA256 pour les mots de passe (avec salt par utilisateur)
- Migration/compatibilité avec ancien SHA256
- Upgrade automatique du hash au login
- Sessions sécurisées avec token stocké en DB
- Limitation essais de connexion + blocage après 5 tentatives en 15 min
- Ne jamais stocker mot de passe en clair dans st.session_state
- Validation minimale mot de passe à l'inscription
"""
...
LEGACY_SALT = "campus_reussite_2024"  # used by legacy sha256 hashing

def pbkdf2_hash(password: str, iterations: int = PBKDF2_ITERATIONS):
...
def pbkdf2_verify(password: str, stored: str) -> bool:
    try:
        _, it_str, salt_hex, dk_hex = stored.split('$', 3)
...
def verify_password(password: str, stored: str) -> bool:
    """
    Support new pbkdf2 format and legacy sha256 bytes.
    - New format: pbkdf2$iterations$salt_hex$hash_hex
    - Legacy: either "sha256$hex" or plain hex (legacy)
    """
    ...
    if stored.startswith("sha256$"):
        # format: sha256$hex
        try:
            _, hexhash = stored.split('$', 1)
            return secrets.compare_digest(hexhash, hashlib.sha256((password + LEGACY_SALT).encode()).hexdigest())
        except Exception:
            return False
    # fallback: plain old hex of legacy sha256
    return secrets.compare_digest(stored, legacy_sha256_hash(password))
...
def strong_password_requirements(pwd: str) -> (bool, str):
    if len(pwd) < 8:
        return False, "Le mot de passe doit contenir au moins 8 caractères"
    if re.search(r'[A-Za-z]', pwd) is None or re.search(r'\d', pwd) is None:
        return False, "Le mot de passe doit contenir au moins une lettre et un chiffre"
    return True, ""

# ========== AUTH / SESSIONS / AUDIT / LOCKOUT ==========
def create_session(utilisateur_id: int):
    token = secrets.token_urlsafe(32)
    # get user's preferred duration
    user = db.fetch_one('SELECT session_duration_minutes FROM utilisateurs WHERE id = ?', (utilisateur_id,))
    minutes = 120
    if user and 'session_duration_minutes' in user:
        try:
            minutes = int(user['session_duration_minutes'])
        except Exception:
            minutes = 120
    expires_at = (datetime.utcnow() + timedelta(minutes=minutes)).isoformat()
    db.query('INSERT INTO sessions (utilisateur_id, token, expires_at) VALUES (?, ?, ?)', (utilisateur_id, token, expires_at))
...
def validate_session_token(token: str):
    if not token:
        return None
    s = db.fetch_one('SELECT * FROM sessions WHERE token = ?', (token,))
    if not s:
        return None
    try:
    ...
    return db.fetch_one('SELECT * FROM utilisateurs WHERE id = ?', (s['utilisateur_id'],))

def audit_log(utilisateur_id, action, details=""):
    try:
        db.query('INSERT INTO audit_logs (utilisateur_id, action, details) VALUES (?, ?, ?)', (utilisateur_id, action, details))
    except Exception:
        pass

def record_failed_login(email: str):
    try:
        db.query('INSERT INTO failed_logins (email) VALUES (?)', (email.lower(),))
    except Exception:
        pass

def count_recent_failed_attempts(email: str, minutes=15):
    since = (datetime.utcnow() - timedelta(minutes=minutes)).isoformat()
    rows = db.fetch_all('SELECT * FROM failed_logins WHERE email = ? AND attempt_time >= ?', (email.lower(), since))
    return len(rows)

def maybe_lock_account_on_too_many_failures(email: str, threshold=5, window_minutes=15):
    if not email:
        return False
    attempts = count_recent_failed_attempts(email, window_minutes)
    if attempts >= threshold:
        # lock account if exists
        user = db.fetch_one('SELECT id, status FROM utilisateurs WHERE email = ?', (email.lower(),))
        if user and user.get('status') != 'bloqué':
            db.query('UPDATE utilisateurs SET status = ? WHERE id = ?', ('bloqué', user['id']))
            audit_log(user['id'], 'account_locked', f'{attempts} failed attempts in {window_minutes} minutes')
        return True
    return False

# ========== FONCTIONS UTILITAIRES ==========
def add_user(nom, prenom, email, username, password):
    """Ajouter un utilisateur avec validation complète"""
    email = email.lower()
    ...
    ok, msg = strong_password_requirements(password)
    if not ok:
        return False, msg
    
    # Vérifier que l'email n'existe pas
    if get_user_by_email(email):
        return False, "email_existe"
    
    # Vérifier que le username n'existe pas
    if db.fetch_one('SELECT id FROM utilisateurs WHERE username = ?', (username,)):
        return False, "username_existe"
    
    try:
        password_hash = pbkdf2_hash(password)
        db.query(
            'INSERT INTO utilisateurs (nom, prenom, email, username, password_hash, role) VALUES (?, ?, ?, ?, ?, ?)',
            (sanitize_input(nom), sanitize_input(prenom), email, username, password_hash, 'apprenant')
        )
        return True, "succes"
...
def toggle_user_status(user_id):
    ...
    if new_status == 'bloqué':
        ...
        audit_log(user_id, 'manual_block', 'admin blocked user')
...
def set_session_duration(user_id, duration):
    if 5 <= duration <= 1440:
        ...
        audit_log(user_id, 'session_duration_changed', f'{duration} minutes')
        return True
    return False
...
def add_quiz_pending(question, a, b, c, d, correct, exp, cat):
    db.query(
        ...
        (sanitize_input(question), sanitize_input(a), sanitize_input(b), sanitize_input(c), sanitize_input(d), correct, sanitize_input(exp), sanitize_input(cat))
    )
...
# If an existing session_token is present, validate it and refresh user info
if st.session_state.get("session_token"):
    validated_user = validate_session_token(st.session_state.get("session_token"))
    if not validated_user:
        ...
        # keep st.session_state.user in sync
        st.session_state.user = validated_user
        st.session_state.logged_in = True
        st.session_state.is_admin = (validated_user.get('role') == 'admin')
...
if not st.session_state.logged_in:
    ...
    with col2:
        ...
        with tab1:
            ...
            email = st.text_input("Email", value=st.session_state.get("pre_fill_email", ""), key="login_email")
            pwd = st.text_input("Mot de passe", type="password", value="", key="login_pwd")  # don't prefill password
            
            if st.button("Se Connecter", use_container_width=True, type="primary"):
                email_val = (email or "").strip().lower()
                # Rate-limit / lock check
                if maybe_lock_account_on_too_many_failures(email_val):
                    st.error("❌ Compte bloqué suite à trop d'essais. Contactez l'administrateur.")
                else:
                    # Vérifier admin (prefer hashed admin in secrets)
                    admin_ok = False
                    try:
                        admins_hashed = st.secrets.get("admins_hashed", {})
                        if isinstance(admins_hashed, dict) and email_val in admins_hashed:
                            # admins_hashed should contain stored password hashes (same format as our stored ones)
                            if verify_password(pwd, admins_hashed[email_val]):
                                admin_ok = True
                        else:
                            # fallback to plaintext admin mapping (legacy)
                            admins = st.secrets.get("admins", {})
                            if isinstance(admins, dict) and email_val in admins and admins[email_val] == pwd:
                                admin_ok = True
                    except Exception:
                        admin_ok = False
                    
                    if admin_ok:
                        # create pseudo-session for admin (id 0)
                        token, expires = create_session(0)
                        st.session_state.session_token = token
                        st.session_state.logged_in = True
                        st.session_state.user = {'nom': 'Admin', 'prenom': 'Campus', 'email': email_val, 'role': 'admin', 'id': 0}
                        st.session_state.is_admin = True
                        audit_log(0, 'admin_login', f'admin {email_val} logged in')
                        st.rerun()
                    ...
                    if user:
                        if user.get('status') == 'bloqué':
                            ...
                            if verify_password(pwd, user['password_hash']):
                                # successful login — upgrade legacy hash if needed
                                if not str(user['password_hash']).startswith("pbkdf2$"):
                                    try:
                                        new_hash = pbkdf2_hash(pwd)
                                        db.query('UPDATE utilisateurs SET password_hash = ? WHERE id = ?', (new_hash, user['id']))
                                        audit_log(user['id'], 'password_upgraded', 'Upgraded legacy hash to pbkdf2')
                                    except Exception:
                                        pass
                                ...
                                # failed
                                record_failed_login(email_val)
                                attempts = count_recent_failed_attempts(email_val)
                                if attempts >= 5:
                                    maybe_lock_account_on_too_many_failures(email_val)
                                    st.error("❌ Trop d'essais — compte bloqué. Contactez l'administrateur.")
                                else:
        ...
        with tab2:
            ...
            if st.button("S'Inscrire", use_container_width=True, type="primary"):
                email_val = (email or "").strip().lower()
                if not all([nom, prenom, email_val, username, password]):
                    ...
                    if not ok:
                        ...
                        success, message = add_user(nom, prenom, email_val, username, password)
                        
                        if success:
                            st.success("✅ Compte créé!")
                            # Pré-remplir uniquement l'email — NE PAS stocker le mot de passe en clair
                            st.session_state.pre_fill_email = email_val
                            st.session_state.pre_fill_password = ""
                            audit_log(None, 'user_created', f'{email_val}')
                            st.info("➡️ Allez à l'onglet 'Connexion' et entrez votre mot de passe pour vous connecter")
                            st.rerun()
                        elif message == "email_existe":
                            st.error("❌ Cet email est déjà utilisé")
                        elif message == "username_existe":
                            st.error("❌ Ce username est déjà utilisé")
                        else:
...
# ========== APPLICATION PRINCIPALE ==========
    ...
    <div class="header-title">
        <h1>🎓 Campus Réussite</h1>
        <p>{'👮 Admin' if is_admin else '👨‍🎓 Apprenant'}: {user.get('prenom','')} {user.get('nom','')}</p>
    </div>
    """, unsafe_allow_html=True)
    
    with st.sidebar:
        st.markdown(f"👤 **{user.get('prenom','')} {user.get('nom','')}**")
        ...
        if st.button("🚪 Déconnexion"):
            # remove session token record
            delete_session_by_token(st.session_state.get("session_token", ""))
            st.session_state.logged_in = False
            st.session_state.user = None
            st.session_state.is_admin = False
            st.session_state.pre_fill_email = ""
                    ...
                    if series_list:
                        ...
                        if st.button("Ajouter", use_container_width=True, type="primary"):
                            for _, row in df.iterrows():
                                add_quiz_pending(row.get('question', ''), row.get('a', ''), row.get('b', ''), row.get('c', ''), row.get('d', ''),
                                               row.get('reponses_correctes', ''), row.get('explication', ''), selected[1])
